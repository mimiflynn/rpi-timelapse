...
def singlephoto():
    ...
    sleep(2)
    camera.capture('/home/pi/Projects/Photos/static/images/single.jpg')
    camera.stop_preview()
    camera.close()
    print('done taking single photo at {:%Y-%m-%d_%H-%M-%S}'.format(datetime.datetime.now()))
    os.system('cp static/images/single.jpg static/images/single-{:%Y-%m-%d_%H-%M-%S}.jpg'.format(datetime.datetime.now()))
...
def timelapse():
    ...
    for i in range(50):
        sleep(5)
        camera.capture('/home/pi/Projects/Photos/static/timelapse/image{0:04d}.jpg'.format(i))
...
def create_gif():
    ...
    os.system('convert -delay 10 -loop 0 static/timelapse/image*.jpg gifs/animation-{:%Y-%m-%d_%H-%M-%S}.gif'.format(datetime.datetime.now()))
